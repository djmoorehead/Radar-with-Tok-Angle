...
const Radar = (() => {
    ...
    */
    ...
    const GetDirectionalInfo = function(who, tokID, tokX, tokY, originX, originY, distType, includeTotalDist, scaleNumber, scaleUnits, gridIncrement, outputCompact, calcType) {
        let retVal = "";
        let xDist;
        let yDist;
        let totalDist;
        ...
        //calculate angle between tokens
        let oPt = new pt(originX, originY);
        let angle = getAngle2TargetToken(who, oPt, tokID);
        
        //Format text output. Default is outputCompact=false
        if (outputCompact) {
            retVal = retVal + ',Dist ' + _h.inlineResult(parseFloat(totalDist).toFixed(fixedDecimals)) + ',Dir ' + _h.inlineResult(parseInt(angle));
        } else {
            retVal = retVal + '<br>' + 'Distance = ' + _h.inlineResult(parseFloat(totalDist).toFixed(fixedDecimals)) + ',Direction ' + _h.inlineResult(parseInt(angle));
        }
                                ...
                                toksInRange.forEach( tok => {
                                    if (tok.filterGroup === filter.groups[i]) {
                                        ...
                                        if (group.indexOf('-')===-1 && (tok.layer === 'gmlayer' || tok.layer === 'walls')) {
                                            ...
                                            content = GetDirectionalInfo(who, tok.id, parseInt(tok.closestPt.x), parseInt(tok.closestPt.y), originX, originY, displayUnits, includeTotalDist, pageScaleNumber, pageScaleUnits, pageGridIncrement, outputCompact, calcType);
                        ...
                        if (toksInRange.length===0) {
                            ...
                            for (let i = 0; i < toksInRange.length; i++) {
                                ...
                                if (toksInRange[i].layer === 'gmlayer' || toksInRange[i].layer === 'walls') {
                                    //identify hidden tokens in chat
                                    content = GetDirectionalInfo(who, toksInRange[i].id, parseInt(toksInRange[i].closestPt.x), parseInt(toksInRange[i].closestPt.y), originX, originY, displayUnits, includeTotalDist, pageScaleNumber, pageScaleUnits, pageGridIncrement, outputCompact, calcType);
